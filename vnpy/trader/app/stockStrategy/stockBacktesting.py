...
class BacktestingEngine(object):
    ...
    def loadHistoryData(self):
        ...
        for symbol in self.symbols:
            self.output(u'载入股票代码%s' % (symbol))
            self._loadHistoryData(symbol)
        self.output(u'载入完成')



    #----------------------------------------------------------------------
    def _loadHistoryData(self, symbol):
        """载入历史数据"""
        ...
        if self.dataLength == 0:
            self.dataLength = dbCursor.count()
        else:
            assert self.dataLength == dbCursor.count()
        ...
        self.strategyData[symbol] = stockData
        self.output(u'载入%s完成，数据量：%s' %(symbol, initCursor.count() + dbCursor.count()))
    ...
    def runBacktesting(self):
        ...
        for i in range(self.dataLength):
            ...
            for symbol in self.symbol:
                data = self.strategyData[symbol][i]
                datas[symbol] = data
